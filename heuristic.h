...
void Satlike::init(vector<int> &init_solution)
{
    ...
    for (int c = 0; c < num_clauses; c++)
    {
        ...
        if (org_clause_weight[c] == top_clause_weight) {
            if (best_soln_feasible == 0) clause_weight[c] = 1 + basis_clause_weight[c];
            else clause_weight[c] = 1;
        }
        else
        {
...
void Satlike::local_search_with_decimation(char *inputfile)
{
    ...
    opt_solu_improved = true;
    for (tries = 1; tries < max_tries; ++tries)
    {
        if (best_soln_feasible != 1)
        {
            deci.init(local_opt_soln, best_soln, unit_clause, unit_clause_count, clause_lit_count, best_soln_feasible, opt_solu_improved, h_inc);
            deci.unit_prosess();
            init(deci.fix);
        }
        else {
